...
public class test<R> implements GJNoArguVisitor<R> {
...
//
   ...
   public R visit(MainClass n) {
      ...
      { 
    	 ...
    	 methods = Class_info_temp.methods;
    	 params = methods.get("main").params;
   ...
   public R visit(ClassDeclaration n) {
	  ...
	  if(flag==0)
	  {
		  ...
		  while(dependee.hasMoreElements())
		  {
			  ...
			  vars = symbol_tab.get(new_dependee).vars;
			  methods = symbol_tab.get(new_dependee).methods;
			  symbol_tab.get(new_dependee).vars = symbol_tab.get(class_name).vars;
			  symbol_tab.get(new_dependee).methods = symbol_tab.get(class_name).methods;
...
//ME -WHENEVER I SEE A NEW CLASS I SHOULD EMPTY ALL MY LinkedHashMapS.
   ...
   public R visit(ClassExtendsDeclaration n) {
      ...
      if(flag==0)
      {
          ...
          list_temp1 = dependent_class.get(class_name); 
      	  System.out.println("class : " + class_name ); 
    	  if(symbol_tab.get(class_name)!=null)
      		System.out.println("error2");  //ME - THROW AN EXCEPTION
    	  Class_info new_class_1 = new Class_info();
    	  symbol_tab.put(class_name,new_class_1);
      ...
      if(flag==0)
      {
	      ...
	      if(symbol_tab.get(class_name_of_extended_class)!=null)
	      {
		    //ME - ADDING THE ALREADY EXISTING CLASS KA LinkedHashMapS TO THE ONE WHICH EXTENDS THIS.
		      ...
		      vars = symbol_tab.get(class_name_of_extended_class).vars;
		      methods = symbol_tab.get(class_name_of_extended_class).methods;
		      Class_info_temp=symbol_tab.get(class_name);
		      Class_info_temp.vars=vars;
		      Class_info_temp.methods=methods;
     ...
     String id = (String) n.f1.accept(this);
      ...
      if(flag==0)
      {
	      ...
	      else
	      {
	    	  ...
	    	  methods = Class_info_temp.methods;
	      	  vars=Class_info_temp.vars;
	      	  params=methods.get(meth_name) .params;
	      	  locals=methods.get(meth_name) .locals;
    ...
    /*  if(flag==0)
      {
		  ...
		  //not allowing over loading.
		  if(symbol_tab.get(class_name).methods.get(meth_name)!=null)
			  System.out.println("error 90");
		  //allowing over riding - 
		  if(symbol_tab.get(parents.get(class_name))!=null)  //parent exists.
		  {
			if(symbol_tab.get(parents.get(class_name)).methods!=null)
				if(symbol_tab.get(parents.get(class_name)).methods.get(meth_name)!=null)
				{
					params = symbol_tab.get(parents.get(class_name)).methods.get(meth_name).params;
					params1 = symbol_tab.get(class_name).methods.get(meth_name).params;
					locals = symbol_tab.get(parents.get(class_name)).methods.get(meth_name).locals;
					String ret_type = symbol_tab.get(parents.get(class_name)).methods.get(meth_name).return_type;
					if(params.size()!=params1.size())
						System.out.println("error45");
					else if(!return_type.equals(ret-type))
     ...
     // n.f1.accept(this);
      ...
      if(flag==0)
      {
	      ...
	      params.put(id, type);
	    	...
	    	//if(class_name.equals("Pranali") && symbol_tab.get("Pranali").methods.get("Comp").params.get("r")!=null)
   ...
   public R visit(AssignmentStatement n) {
      ...
      else
      {
    	  ...
    	  String parent_type = new String();
    	  parent_type = parents.get(type1) ;
    	  System.out.println("parent type is : "+ parent_type);
    	  if(!type2.equals(type1) && !parent_type.equals(type2))
    	  {
    	...
    	/*  while(!stack2.empty())
    	  ...
    	  */
    	  ...
    	  if(symbol_tab.get(class_name).vars.get(new_object_called)!=null) // a new class type which was previously an object.
    	  {
    		 ...
    		 int param_count=0;
    		 int flag2=0;
    		 params = symbol_tab.get(new_caller_type).methods.get(new_method_called).params;
    		 while(!stack2.empty())
    		 {
    	  ...
    	  else if (in_this==1) //this wala method called;
    	  {
    		  ...
    		  int param_count=0;
    		  int i=0;
    		  params = symbol_tab.get(class_name).methods.get(new_method_called).params;
    		  while(!stack2.empty())
     		 {
    	  ...
    	  else if(in_new==1)
    	  {
    		  ...
    		  int param_count=0;
    		  int i=0;
    		  params = symbol_tab.get(new_object_called).methods.get(new_method_called).params;
    		  while(!stack2.empty())
     		 {
